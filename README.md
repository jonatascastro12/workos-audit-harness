...
It previously lived in its own repo (`workos/workos-audit-chat`, now archived) because the `wow` CI pipeline deploys repo-per-app. It was folded in here with its history preserved, so everything that reads or writes this audit trail versions together.
