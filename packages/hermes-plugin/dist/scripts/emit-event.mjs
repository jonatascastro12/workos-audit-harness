...
import { chmodSync, existsSync as existsSync2, mkdirSync, readFileSync as readFileSync2, rmSync as rmSync2, writeFileSync as writeFileSync2 } from "node:fs";
var CONFIG_KEYS = [
  "apiKey",
  "organizationId",
  "actionPrefix",
...
function getManagedConfigPath() {
  ...
  if (process.platform === "win32") {
    return path2.join(process.env.PROGRAMDATA || "C:\\ProgramData", "workos-audit", "config.json");
  }
...
`, { mode: 384 });
  ...
  function loadConfig() {
    ...
    const actorId = resolveKey("actorId", fileConfig, managedConfig, () => {
      ...
      return { value: os2.hostname(), source: "hostname" };
    });
...
var SHELL_TOOL_NAMES = new Set(["terminal", "bash", "shell"]);
var FAILURE_STATUSES = new Set(["error", "failed", "failure", "timeout", "denied"]);
...
function isToolFailure(payload) {
  if (typeof payload.error_type === "string" && payload.error_type)
    return true;
  if (typeof payload.status === "string" && FAILURE_STATUSES.has(payload.status.toLowerCase()))
    return true;
  return payload.is_error === true;
}
function resolveKind(kind, payload) {
  if (kind === "tool-finished")
    return isToolFailure(payload) ? "tool-failed" : "tool-completed";
  if (kind === "turn-finished")
    return payload.failed === true || payload.interrupted === true ? "turn-failed" : "turn-completed";
  return kind;
}
function getSessionId(kind, payload) {
  if (kind === "agent-started" || kind === "agent-completed")
    return payload.parent_session_id || payload.session_id;
  if (kind === "permission-requested" || kind === "permission-resolved")
    return payload.session_key || payload.session_id;
...
function historyMessageCount(payload) {
  if (typeof payload.history_message_count === "number")
    return payload.history_message_count;
...
function commandFields(payload) {
  ...
  if (!SHELL_TOOL_NAMES.has(payload.tool_name))
    return {};
  const command = payload.args && typeof payload.args === "object" && typeof payload.args.command === "string" ? payload.args.command : undefined;
...
function goalFields(payload) {
  ...
  if (typeof payload.child_goal !== "string")
    return {};
  return {
...
function summaryFields(payload) {
  ...
  if (typeof payload.child_summary !== "string")
    return {};
  return {
...
function toolCallHistoryFields(payload) {
  ...
  if (!Array.isArray(payload.tool_call_history))
    return {};
...
function buildMetadata(kind, payload) {
  ...
  if (kind === "permission-requested" || kind === "permission-resolved") {
    return compactMetadata({
      ...
      ...kind === "permission-resolved" ? { choice: payload.choice, decided_by: payload.decided_by } : {},
      ...commonMetadata(payload)
    });
  }
