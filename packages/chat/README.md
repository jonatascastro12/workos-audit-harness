...
The app represents the same WorkOS tenant/environment the audit ingestion proxy (`cd26-workos-audit-proxy`, deployed from [`packages/proxy`](../proxy)) writes into: it reuses the proxy's `AUDIT_HARNESS_WORKOS_API_KEY` / `AUDIT_HARNESS_WORKOS_ORG_ID` secrets unless overridden.
...
- **Proxy settings** (`/settings`) — admins manage the ingestion proxy's runtime settings: pause/resume ingestion (with reason and auto-resume), the WorkOS organization events are attributed to, the Kandji device-cache TTL, the unassigned-device policy, and the cached device→user mappings (view/purge). Settings are written as one JSON document (`app_state` key `proxy_settings`) directly into the proxy's D1 database via the `PROXY_DB` binding — no HTTP admin API. The proxy reads the document per ingested event and falls back to its env vars for absent/malformed fields, so changes apply without a redeploy and a missing row (or D1 outage) never breaks ingest. The write contract lives in `app/lib/proxy-settings.server.ts`; the proxy's tolerant reader is `packages/proxy/src/settings.ts`. Note the kill switch fails **open** during a proxy-DB outage by design. The "env default" shown for the organization is **this app's** copy of `AUDIT_HARNESS_WORKOS_ORG_ID`; the proxy reads its own copy — both come from the shared `claude-day` Doppler project, but if they ever diverge the label may not match what the proxy actually uses.
  - **Local dev:** `PROXY_DB` binds to a separate empty miniflare copy, so `/settings` renders env defaults read-only. To exercise writes, apply the proxy's migrations to the local copy: `npx wrangler d1 execute cd26-workos-audit-proxy-db --local --file ../proxy/migrations/0001_create_device_user.sql` (repeat for `0002_create_app_state.sql`).
