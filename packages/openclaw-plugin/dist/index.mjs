...
var require_eventemitter3 = __commonJS((exports, module3) => {
  ...
  switch (alg) {
    ...
    case "ECDH-ES+A256KW": {
      ...
      if (kty === "EC")
        parameters3.epk.y = y;
      if (apu)
        parameters3.apu = encode2(apu);
      if (apv)
        parameters3.apv = encode2(apv);
      if (alg === "ECDH-ES") {
        cek = sharedSecret;
        break;
      }
    ...
    case "PBES2-HS512+A256KW": {
      ...
      ({ encryptedKey, ...parameters3 } = await wrap$1(alg, key, cek, p2c, p2s));
      break;
    }
    case "A128KW":
    case "A192KW":
    ...
    case "A256GCMKW": {
      ...
      ({ encryptedKey, ...parameters3 } = await wrap(alg, key, cek, iv));
      break;
    }
    default:
      throw new JOSENotSupported(unsupportedAlgHeader);
...
function validateCrit(Err, recognizedDefault, recognizedOption, protectedHeader, joseHeader) {
  ...
  for (const parameter2 of protectedHeader.crit) {
    if (!recognized.has(parameter2))
      throw new JOSENotSupported(`Extension Header Parameter "${parameter2}" is not recognized`);
    if (joseHeader[parameter2] === undefined)
      throw new Err(`Extension Header Parameter "${parameter2}" is missing`);
    if (recognized.get(parameter2) && protectedHeader[parameter2] === undefined)
...
async function flattenedVerify(jws, key, options) {
  ...
  const data = concat(jws.protected !== undefined ? encode$1(jws.protected) : new Uint8Array, encode$1("."), typeof jws.payload === "string" ? b64 ? encode$1(jws.payload) : encoder2.encode(jws.payload) : jws.payload);
...
var __defProp2, __exportAll = (all, no_symbols) => {
  let target2 = {};
  for (var name in all)
    __defProp2(target2, name, {
      get: all[name],
      enumerable: true
    });
  if (!no_symbols)
    __defProp2(target2, Symbol.toStringTag, { value: "Module" });
  return target2;
}, encoder2, decoder, MAX_INT32, base64url_exports, unusable = (name, prop = "algorithm.name") => /* @__PURE__ */ new TypeError(`CryptoKey does not support this operation, its ${prop} must be ${name}`), isAlgorithm = (algorithm, name) => algorithm.name === name, invalidKeyInput = (actual, ...types2) => message("Key must be ", actual, ...types2), withAlg = (alg, actual, ...types2) => message(`Key for the ${alg} algorithm must be `, actual, ...types2), errors_exports, JOSEError, JWTClaimValidationFailed, JWTExpired, JOSEAlgNotAllowed, JOSENotSupported, JWEDecryptionFailed, JWEInvalid, JWSInvalid, JWTInvalid, JWKInvalid, JWKSInvalid, JWKSNoMatchingKey, JWKSMultipleMatchingKeys, JWKSTimeout, JWSSignatureVerificationFailed, isCryptoKey = (key) => {
...
-----END ${descriptor}-----`;
  ...
  setKeyManagementParameters(parameters3) {
    assertNotSet(this.#keyManagementParameters, "setKeyManagementParameters");
    this.#keyManagementParameters = parameters3;
    return this;
  }
  ...
  async encrypt(key, options) {
    ...
    let cek;
    {
      ...
      if (parameters3)
        if (options && unprotected in options)
          ...
          this.setProtectedHeader(parameters3);
        else
          this.#protectedHeader = {
            ...this.#protectedHeader,
            ...parameters3
    ...
    const jwe = { ciphertext: encode2(ciphertext) };
    if (iv)
      jwe.iv = encode2(iv);
    if (tag2)
  ...
  async encrypt() {
    ...
    for (let i = 0;i < this.#recipients.length; i++) {
      ...
      const { encryptedKey, parameters: parameters3 } = await encryptKeyManagement(alg, enc, k, cek, recipient.keyManagementParameters);
      target2.encrypted_key = encode2(encryptedKey);
      if (recipient.unprotectedHeader || parameters3)
        target2.header = {
          ...recipient.unprotectedHeader,
          ...parameters3
        };
  ...
  async sign(key, options) {
    ...
    const jws = {
      signature: encode2(await sign(alg, await normalizeKey(key, alg), data)),
      payload: payloadS
    };
    if (this.#unprotectedHeader)
  ...
  async sign() {
    ...
    for (let i = 0;i < this.#signatures.length; i++) {
      ...
      const { payload, ...rest3 } = await flattened.sign(signature.key, signature.options);
      if (i === 0)
        jws.payload = payload;
      else if (jws.payload !== payload)
        throw new JWSInvalid("inconsistent use of JWS Unencoded Payload (RFC7797)");
      jws.signatures.push(rest3);
    }
    return jws;
  }
}, SignJWT = class {
  ...
  async getKey(protectedHeader, token) {
    ...
    const candidates2 = this.#jwks.keys.filter((jwk2) => {
    ...
    const { 0: jwk, length } = candidates2;
    if (length === 0)
  ...
  encode() {
    return `${encode2(JSON.stringify({ alg: "none" }))}.${encode2(this.#jwt.data())}.`;
  }
  setIssuer(issuer) {
    this.#jwt.iss = issuer;
    return this;
...
var unreachable2 = (condition, message2 = `Entered unreachable code. Received '${condition}'.`) => {
...
var deserializeAuthenticationResponse = (authenticationResponse) => {
  const { user, organization_id, access_token, refresh_token, authentication_method, impersonator, oauth_tokens, ...rest3 } = authenticationResponse;
  return {
...
var serializeCreateAuditLogSchemaOptions = (schema2) => ({
  actor: { metadata: {
    type: "object",
    properties: serializeMetadata(schema2.actor?.metadata)
  } },
  targets: schema2.targets.map((target2) => {
    return {
      type: target2.type,
      metadata: target2.metadata ? {
        type: "object",
        properties: serializeMetadata(target2.metadata)
      } : undefined
    };
  }),
...
var deserializeAuditLogSchema = (auditLogSchema) => ({
  ...
  targets: auditLogSchema.targets.map((target2) => {
    return {
      type: target2.type,
      metadata: target2.metadata ? deserializeMetadata(target2.metadata) : undefined
    };
  }),
...
function jsonParse(string3) {
  try {
    return JSON.parse(string3);
  } catch (err) {
...
function isJson(val) {
  ...
  for (;stack.length; ) {
    ...
    if (proto !== null && proto !== Object.prototype)
      return false;
    let keys2 = Reflect.ownKeys(obj), i = keys2.length;
    for (;i--; ) {
      let key = keys2[i];
...
var deserializeObject = (object3) => ({
  id: object3.id,
  name: object3.name,
  ...object3.value !== undefined && { value: object3.value },
  metadata: deserializeObjectMetadata(object3.metadata)
});
...
var Vault = class {
  ...
  async encrypt(data, context, associatedData) {
    ...
    const aadBuffer = associatedData ? encoder3.encode(associatedData) : undefined;
    const iv = this.cryptoProvider.randomBytes(12);
    const { ciphertext, iv: resultIv, tag: tag2 } = await this.cryptoProvider.encrypt(encoder3.encode(data), key, iv, aadBuffer);
...
function readManagedConfig() {
  ...
  if (!existsSync3(filePath))
    return {};
  try {
    return sanitizeRawConfig(JSON.parse(readFileSync3(filePath, "utf8")));
  } catch {
    return {};
  }
}
...
function createConfigLoader({
  ...
  function getConfigFilePath() {
    ...
    return path4.join(os4.homedir(), defaultConfigDir, "workos-audit", "config.json");
  }
...
`, { mode: 384 });
  ...
  function loadConfig() {
    ...
    const actorId = resolveKey("actorId", fileConfig, managedConfig, () => {
      ...
      return { value: os4.hostname(), source: "hostname" };
    });
...
var workspaceIdentityCache = { loaded: false, value: {} };
...
function workspaceIdentity() {
  if (workspaceIdentityCache.loaded)
    return workspaceIdentityCache.value;
  workspaceIdentityCache.loaded = true;
  const workspaceDir = pick3(process.env.OPENCLAW_WORKSPACE_DIR, process.env.OPENCLAW_WORKSPACE, path5.join(os5.homedir(), ".openclaw", "workspace"));
  try {
    workspaceIdentityCache.value = {
      assistantName: readMarkdownField(path5.join(workspaceDir, "IDENTITY.md"), "Name"),
      userName: readMarkdownField(path5.join(workspaceDir, "USER.md"), "Name")
    };
  } catch {
    workspaceIdentityCache.value = {};
  }
...
function actorFrom(payload = {}, context = {}, config = {}) {
  ...
  const explicitActorId = !["os_user", "hostname", "default"].includes(config.sources?.actorId) ? config.actorId : undefined;
  const explicitActorType = !["default"].includes(config.sources?.actorType) ? config.actorType : undefined;
  const explicitActorName = !["os_user", "hostname", "default"].includes(config.sources?.actorName) ? config.actorName : undefined;
  const agentId = pick3(payload.agentId, payload.agent_id, context.agentId, context.agent_id);
  const assistantName = pick3(payload.agentName, payload.agent_name, context.agentName, context.agent_name, identity.assistantName, agentId, "OpenClaw");
  const userName = pick3(payload.senderName, payload.sender_name, payload.fromName, payload.from_name, context.senderName, context.sender_name, identity.userName);
  const userId = pick3(payload.senderId, payload.sender_id, payload.from, context.senderId, context.sender_id);
  const idParts = [slug(assistantName), slug(userName || userId)].filter(Boolean);
  const metadata = compactMetadata({
    machine_user: pick3(process.env.USER, process.env.USERNAME),
...
function sessionTarget(event = {}, context = {}) {
  ...
  const fallback = compactMetadata({
    ...
    job_id: pick3(event.jobId, event.job_id, context.jobId, context.job_id),
    account_id: pick3(event.accountId, event.account_id, context.accountId, context.account_id),
    channel_id: pick3(event.channelId, event.channel_id, context.channelId, context.channel_id, event.chatId, event.chat_id, context.chatId, context.chat_id),
    thread_id: pick3(event.threadId, event.thread_id, context.threadId, context.thread_id),
    provider: pick3(event.messageProvider, event.message_provider, context.messageProvider, context.message_provider),
    agent_id: pick3(event.agentId, event.agent_id, context.agentId, context.agent_id)
  });
...
var openclaw_plugin_default = definePluginEntry({
  ...
  register(api) {
    ...
    api.registerTool({
      ...
      parameters: exports_typebox.Object({
        question: exports_typebox.String(),
        rangeStart: exports_typebox.Optional(exports_typebox.String()),
        rangeEnd: exports_typebox.Optional(exports_typebox.String()),
        actions: exports_typebox.Optional(exports_typebox.Array(exports_typebox.String())),
        actorIds: exports_typebox.Optional(exports_typebox.Array(exports_typebox.String())),
        actorNames: exports_typebox.Optional(exports_typebox.Array(exports_typebox.String())),
        targets: exports_typebox.Optional(exports_typebox.Array(exports_typebox.String())),
        maxRows: exports_typebox.Optional(exports_typebox.Number({ minimum: 1, maximum: MAX_QUERY_MAX_ROWS }))
      }),
