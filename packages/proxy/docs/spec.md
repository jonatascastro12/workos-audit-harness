...
## 5. Proxy implementation (Cloudflare Worker via `wow` internal CLI)

The proxy is a WorkOS internal app scaffolded by `wow app create`. `wow`
...
```bash
wow app create workos-audit-proxy
```
...
- `WORKOS_API_KEY` — the real full-access `sk_`. Store as a **Doppler secret**;
  `wow`/GH Actions syncs it into the Worker env. **Never** in the repo,
  `wrangler.toml`, or on any laptop.
- `WORKOS_ORG_ID` — Doppler config var.

### 5.5 Ingress — Cloudflare Zero Trust mTLS on the ingest path

`wow` puts the whole app behind Access SSO by default. That would block headless
...
> `wow` has no built-in capability for mTLS (its only add-on is
> `wow app add slack`, which creates an Access *bypass* for `/api/slack`). The
> mTLS setup above is a manual one-time Cloudflare step. Consider proposing a
> `wow app add mtls` capability as a follow-up.
...
The `wow` React frontend (behind normal Access SSO) can show ingest
